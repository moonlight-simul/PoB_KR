...
function launch:CheckForUpdateCN(inBackground)
	if self.updateCheckRunning then
		return
	end
	self.updateCheckBackground = inBackground
	self.updateMsg = "Initialising2..."
	self.updateProgress = "Checking2..."
	self.lastUpdateCheck = GetTime()
	local update = io.open("ZdDroid/UpdateCheckCN.lua", "r")
